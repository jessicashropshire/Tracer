...
class DishScene(TracerScene):
    ...
    disp_num_rays = t_api.Int(500)# why an integer
    receiver_pos = t_api.Float(1)# a single numer on the z axis
    receiver_side=t_api.Float(0.2)# the dimension of the side of a square receiver
    ...
    def create_dish_source(self):
        ...
        """
        diameter=1.
        dish, f, H = petal_dish(diameter, self.receiver_pos, self.receiver_side, self.refl, 1.)
	print(f,'f')
	...
	#recv = AssembledObject(surfs=[receiver_surf])parabolic_dish_jess_3.py
