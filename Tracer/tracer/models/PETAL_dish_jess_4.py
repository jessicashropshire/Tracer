...
import numpy as N
from .Two_N_parameters_cavity_jess import TwoNparamcav
...
class PETAL(TwoNparamcav):
  ...
  # dish = PETAL(1., self.f, self.H, self.receiver_size, self.w, homog_opt_eff=0.9,dish_opt_eff=0.9, self.receiver_aspect=1)
	...
	recv=TwoNparamcav(apertureRadius=N.array([0.22]), frustaRadii=N.array([0.75]), frustaDepths=N.array([0.2]), coneDepth=1., eps_wall=0.04, absReceiver=0.9, emsReceiver=0.2, aperture_position=receiver_pos, envelope_radius=0.23, envelope_depth = None, specular_receiver=False)
	
	#TwoNparamcav. __init__(self, apertureRadius=N.array([0.22,0.23,0.24,0.25,0.26]), frustaRadii=N.array([0.75,0.76,0.77,0.78,0.79]), frustaDepths=N.array([0.2]), coneDepth=1., eps_wall=0.04, absReceiver=0.9, emsReceiver=0.2, aperture_position=receiver_pos, envelope_radius = [0.23,0.24,0.25,0.26,0.27], envelope_depth = None, specular_receiver=False)
	'''
	...
	#recv=TwoNparamcav(N.array([0.22]), N.array([0.75]), N.array([0.2]), 1., 0.04, 0.9, 0.2, receiver_pos, 0.23, None, False)
	self.refl= AssembledObject(surfs=[dish_surf])
	self.add_object(recv)
	# it is in itsself an assembly
	#recv = AssembledObject(surfs=[receiver_surf])
	
	#_init__(self, apertureRadius, frustaRadii, frustaDepths, coneDepth, eps_wall, absReceiver, emsReceiver, aperture_position, envelope_radius = None, envelope_depth = None, specular_receiver=False):
	...
	#Assembly.__init__(self,objects=[recv])
    ...
    def get_main_reflector(self):
        return self._mr
def petal_dish(diameter, receiver_pos, receiver_side, dish_opt_eff,concent,receiver_aspect=1.):
	virt_sources=1.
	f = diameter/4./(N.sqrt(2.) - 1.)
	W = diameter/2. * N.sqrt(N.pi/concent)
   	n = virt_sources + 1.
   	H = n*W*f/(diameter - n*W)
	H = receiver_pos-f
