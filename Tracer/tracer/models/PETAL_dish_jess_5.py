...
class PETAL(TwoNparamcav):
  ...
  # dish = PETAL(1., self.f, self.H, self.receiver_size, self.w, homog_opt_eff=0.9,dish_opt_eff=0.9, self.receiver_aspect=1)
	...
	recv_1=TwoNparamcav(apertureRadius=N.array([0.22]), frustaRadii=N.array([0.75]), frustaDepths=N.array([0.2]), coneDepth=1., eps_wall=0.04, absReceiver=0.9, emsReceiver=0.2, aperture_position=0.6, envelope_radius=0.23, envelope_depth = None, specular_receiver=False)
	#recv_surf=Surface(recv, opt.LambertianReceiver(0.9))
	recv=Assembly(objects=[recv_1])
	refl= AssembledObject(surfs=[dish_surf])
	...
	Assembly.__init__(self,objects=[recv,refl])
