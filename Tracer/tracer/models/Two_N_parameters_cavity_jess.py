...
class TwoNparamcav(Assembly):
	...
	def __init__(self, apertureRadius, frustaRadii, frustaDepths, coneDepth, eps_wall, absReceiver, emsReceiver, aperture_position, envelope_radius = None, envelope_depth = None, specular_receiver=False):
		...
		#AssembledObject.__init__(self, objects=None, subassemblies=None, location=None)	
		...
		if self.envelope_radius != self.apertureRadius:    # Here the argument returns a list of boolean values. Which might be difficult to pass on to if statement
			Envelope_front = AssembledObject(surfs=[Surface(RoundPlateGM(Ri=self.apertureRadius, Re=self.envelope_radius), RealReflectiveReceiver(1,sigma_xy))], transform=None)
			Envelope.add_object(Envelope_front)

		Envelope_back = AssembledObject(surfs=[Surface(RoundPlateGM(Re=self.envelope_radius), RealReflectiveReceiver(1,sigma_xy))], transform=translate(z=self.envelope_depth))
		Envelope_cylinder = AssembledObject(surfs=[Surface(FiniteCylinder(diameter=2.*self.envelope_radius, height=self.envelope_depth), RealReflectiveReceiver(1,sigma_xy))], transform=translate(z=self.envelope_depth/2.))
		...
		if specular_receiver==False:		
			...
			if self.coneDepth < 0.: # self.coneDepth < 0 Inward cone
				CON = AssembledObject(surfs=[Surface(FiniteCone(r=self.frustaRadii[-1], h=-self.coneDepth), RealReflectiveReceiver(self.absReceiver[-1],sigma_xy))], transform=translate(z=N.sum(frustaDepths)+self.coneDepth))
	
			elif self.coneDepth == 0.: # Round flat plates
				CON = AssembledObject(surfs=[Surface(RoundPlateGM(Re=self.frustaRadii[-1]), RealReflectiveReceiver(self.absReceiver[-1],sigma_xy))], transform=translate(z=N.sum(frustaDepths)))
				...
				CON = AssembledObject(surfs=[Surface(FiniteCone(r=self.frustaRadii[-1], h=self.coneDepth), RealReflectiveReceiver(self.absReceiver[-1],sigma_xy))], transform=trc)

			FRU = []
			# 1st frustum:
			if self.apertureRadius==self.frustaRadii[0]: # Cylinder
				frustum = AssembledObject(surfs=[Surface(FiniteCylinder(diameter=self.apertureRadius*2, height=self.frustaDepths[0]), RealReflectiveReceiver(self.absReceiver[1],sigma_xy))], transform=translate(z=self.frustaDepths[0]/2.))
			elif self.frustaDepths[0] == 0.:
				frustum = AssembledObject(surfs=[Surface(RoundPlateGM(Re=self.apertureRadius, Ri=self.frustaRadii[0]), RealReflectiveReceiver(self.absReceiver[1],sigma_xy))], transform=translate(z=N.sum(self.frustaDepths[:i])))
			else:
				frustum = AssembledObject(surfs=[Surface(ConicalFrustum(z1=0., r1=self.apertureRadius[0], z2=self.frustaDepths[0], r2=self.frustaRadii[0]), RealReflectiveReceiver(self.absReceiver[1],sigma_xy))], transform=None)
			FRU.append(frustum)
			# next frusta:
			for i in xrange(1,len(frustaRadii)):
				if self.frustaRadii[i-1]==self.frustaRadii[i]: # Cylinder
					frustum = AssembledObject(surfs=[Surface(FiniteCylinder(diameter=self.frustaRadii[i-1]*2, height=self.frustaDepths[i]), RealReflectiveReceiver(self.absReceiver[1+i],sigma_xy))], transform=translate(z=N.sum(self.frustaDepths[:i])+self.frustaDepths[i]/2.))
				elif self.frustaDepths[i] < 0.:
					frustum = AssembledObject(surfs=[Surface(ConicalFrustum(z1=0., r1=self.frustaRadii[i-1], z2=-self.frustaDepths[i], r2=self.frustaRadii[i]), RealReflectiveReceiver(self.absReceiver[1+i]))], transform=N.dot(translate(z=N.sum(self.frustaDepths[:i])),rotx(N.pi)))
				elif self.frustaDepths[i] > 0.:
					frustum = AssembledObject(surfs=[Surface(ConicalFrustum(z1=0., r1=self.frustaRadii[i-1], z2=self.frustaDepths[i], r2=self.frustaRadii[i]), RealReflectiveReceiver(self.absReceiver[1+i],sigma_xy))], transform=translate(z=N.sum(self.frustaDepths[:i])))
				else:
					frustum = AssembledObject(surfs=[Surface(RoundPlateGM(Re=self.frustaRadii[i-1], Ri=self.frustaRadii[i]), RealReflectiveReceiver(self.absReceiver[-1]),sigma_xy)], transform=translate(z=N.sum(self.frustaDepths[:i])))
